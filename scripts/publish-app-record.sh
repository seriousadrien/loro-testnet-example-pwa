...
if [ -z "$CERC_REGISTRY_APP_CRN" ]; then
  ...
  AUCTION_ID=$(laconic -c $CONFIG_FILE registry autority whois ${authority})
  echo 'AUCTION_ID'
  echo $AUCTION_ID
  REVEAL_FILE=$(laconic -c $CONFIG_FILE registry auction bid commit $AUCTION_ID 5000000 alnt | jq -r '.reveal_file')
  echo 'REVEAL_FILE'
  echo $REVEAL_FILE
  laconic -c $CONFIG_FILE registry auction bid reveal $AUCTION_ID $REVEAL_FILE
  ...
  laconic -c $CONFIG_FILE registry authority whois ${authority}
fi
