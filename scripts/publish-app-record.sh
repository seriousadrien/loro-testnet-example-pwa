...
if [ -z "$CERC_REGISTRY_BOND_ID" ]; then
  bond_id=$(laconic -c $CONFIG_FILE registry bond create --type alnt --quantity 9000000000 --user-key "${CERC_REGISTRY_USER_KEY}")
...
if [ -z "$CERC_REGISTRY_APP_CRN" ]; then
  ...
  laconic -c $CONFIG_FILE registry authority whois ${authority} --user-key "${CERC_REGISTRY_USER_KEY}"
fi
